...
jQuery.fn.floatdialog = function(id, options)
{
   var Config = {
    ...
    function display_mask(id)
    {
        if(Config.effect)
        {
            $("#floatdialog_mask_" + id).css({'width':$(document).width() + "px", 'height':$(document).height() + "px"}).show().fadeTo(Config.speed, 0.8);
        }
        else
        {
            $("#floatdialog_mask_" + id).css({'width':$(document).width() + "px", 'height':$(document).height() + "px", opacity: '0.8'}).show();
        }
    }
    function center_form(id)
    {
          ...
          //var scTop = (window.navigator.userAgent.indexOf('Chrome') == -1) ? brwstester().scrollTop : (document.documentElement.scrollTop+document.body.scrollTop);
          var scTop = (document.documentElement ? document.documentElement.scrollTop : 0) + (document.body ? document.body.scrollTop : 0);


          if(Config.top == 'default')
          {
                var top = (scTop + ((scheight/3) - (tblheight/2)));
          }
          else
          {
                var top = scTop + parseInt(Config.top);
          }
...
function center_form(id)
{
      ...
      var scheight = brwstester().scrollTop;
      var tblwidth = $("#" + id).width();
      var tblheight = $("#" + id).height();

      if(Config.top == 'default')
      {
            var top = scheight + (document.body.clientHeight - tblheight)/3;
            alert(top);
      }
      else
      {
      ...
      if(Config.left == 'default')
      {
            var left = (((scwidth-brwstester().scrollLeft - tblwidth)/3));
      }
      else
      {
            var left = parseInt(Config.left);
